        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if (nextx, nexty) in self.corners and (nextx, nexty) not in state[1]:
                    nextCorners.append( (nextx, nexty))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    cord, visited = state[0], set(state[1])
    unvisited = set(corners) - visited
    ...
    for _ in range(len(unvisited)):
        ...
        for corner in unvisited:
            distance.append((corner, util.manhattanDistance(cord, corner)))

        cord, dist_min = min(distance, key=lambda x: x[1])

        value += dist_min

        unvisited.remove(cord)
